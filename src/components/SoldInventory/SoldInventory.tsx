...
// Style
import CSS from './SoldInventory.module.scss'


export default function SoldInventory({ cars, blok }: {
    cars: Array<any>,
    blok: any,
}) {
    ...
    useEffect(() => {
        if (model || make || minYear || maxYear || fuelType) {
            let newFilteredCars = [];
            cars.forEach(loop => {
                if (
                    (loop.make === make || !make) &&
    ...
    }, [model, make, minYear, maxYear, fuelType])
    ...
    useEffect(() => {
        if (showMaxAmount !== 20) {
            sessionStorage.setItem("currentMaxSoldInventoryShownAmount", `${showMaxAmount}`);
        }
    }, [showMaxAmount]);

    // Get session storage for max amount
    useEffect(() => {
        const session = sessionStorage.getItem("currentMaxSoldInventoryShownAmount");
        if (session) {
                ...
                <ul className={CSS.cars}>
                    ...
                    {(!filteredCars || filteredCars.length === 0) && 
                        <p style={{paddingBottom: '5em'}}>
                            ...
                            <button className={CSS.reset} style={{borderBottom: 'unset'}} onClick={() => { setFilteredCars(cars); setShow(null); setModel(null); setMake(null); setMinYear(null); setMaxYear(null); setFuelType(null)}}>
                                Reset all filters
